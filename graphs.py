...
def get_food_subsidy_graph():
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["subsidy.released.for.current.year"],
    ...
    fig.add_trace(
        go.Scatter(
            x=df["year"].astype(str),
            y=df["subsidy.incurred.current.year"],
    ...
    fig.update_layout(
        title={
            "text": "Subsidy Released For Current Year Vs Subsidy Incurred in Current Year"
        },
...
def get_year_wise_total_ao_graph():
    ...
    df_rice = (
        df_rice.groupby("year")
        .agg(sum)
        .reset_index()
        .rename({"offtake": "rice_offtake", "allotment": "rice_allotment"}, axis=1)
    )

    df_wheat = pd.read_excel("src/data/wheat.xlsx")
    df_wheat.drop(["State.UT", "zone"], axis=1, inplace=True)
    df_wheat = (
        df_wheat.groupby("year")
        .agg(sum)
        .reset_index()
        .rename({"offtake": "wheat_offtake", "allotment": "wheat_allotment"}, axis=1)
    )
    ...
    df_total.drop(
        {"rice_allotment", "wheat_allotment", "rice_offtake", "wheat_offtake"},
        axis=1,
        inplace=True,
    )
