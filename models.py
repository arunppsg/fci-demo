    ...
    for state in states:
        ...
        for year in range(2012, 2020):
            ...
            new_entry = pd.DataFrame(
                {"State.UT": [state], "percent": [perc], "year": [year]}
            )
...
def all_pred_data(
    ...
    fut_data["Wheat_Allotment"] = wheat_bpl_fit.predict(
        fut_data[["Population", "bpl_pop", "wheat_perc"]]
    )
...
def bpl_population_plot(vis):
    ...
    wheat_bpl_fit = linear_model.LinearRegression().fit(
        rp[["Population", "bpl_pop", "wheat_moving_perc"]], rp["wheat_allotment"]
    )
